...
@section('content')
    ...
    @forelse ($days as $day)
        @php
            ...
            // Замовлений день теж не можна додати повторно — інакше харчування
            // дублювалося б. Спершу треба скасувати попереднє замовлення.
            $alreadyOrdered = in_array($day->date->toDateString(), $datesOrdered, true);
        @endphp
        ...
        <details @class([
            ...
            <summary class="flex cursor-pointer list-none flex-wrap items-center justify-between gap-2 px-4 py-3.5
                ...
                <span class="flex items-center gap-2.5">
                    ...
                    @if ($alreadyOrdered)
                        <span class="badge inline-flex items-center gap-1 bg-emerald-600 text-white">
            ...
            <form method="POST" action="{{ route('cart.store-day', [$supplier->slug, $day->date->toDateString()]) }}"
                ...
                {{-- Доданий чи вже замовлений день теж блокуємо: міняти вибір
                    ...
                    <div class="border-t border-ink-100 bg-ink-50/60 px-4 py-4">
                        @if ($alreadyOrdered)
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                <span class="flex items-center gap-2 text-sm font-medium text-emerald-800">
                                    <svg class="h-5 w-5 shrink-0 text-emerald-700" viewBox="0 0 24 24" fill="none"
                                         stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                         stroke-linejoin="round" aria-hidden="true">
                                        <circle cx="12" cy="12" r="10"/><path d="M20 6 9 17l-5-5"/>
                                    </svg>
                                    Цей день уже замовлено
                                </span>

                                <a href="{{ route('orders.index') }}" class="btn-accent">
                                    Мої замовлення
