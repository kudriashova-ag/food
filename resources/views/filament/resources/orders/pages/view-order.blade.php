@php
    ...
    $facts = [
        'Учень / вчитель' => $student->full_name,
        ...
        <x-filament::section>
            ...
            <div class="flex flex-col gap-6">
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-y border-gray-200 bg-gray-50/50 dark:border-white/10 dark:bg-white/5">
                                        ...
                                        <th class="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Постачальник</th>
                                    ...
                                    @foreach ($lines as $line)
                                        <tr @class(['text-gray-400 dark:text-gray-500' => $line->isCancelled()])>
                                            <td class="px-4 py-2.5">
                                                ...
                                                @if ($line->isCancelled())
                                                    <span class="text-xs">
                                                        (скасовано {{ $line->cancelled_at?->translatedFormat('d.m H:i') }}@if ($line->cancel_reason): {{ $line->cancel_reason }}@endif)
                                                    </span>
                                                @endif
                                            </td>
                                ...
                                <tfoot>
                                    <tr class="border-t border-gray-200 bg-gray-50/50 dark:border-white/10 dark:bg-white/5">
                                        <td colspan="4" class="px-4 py-2 text-right text-gray-500 dark:text-gray-400">
                                            Разом за день
                                        </td>
                ...
                <div class="flex items-baseline justify-between rounded-xl bg-gray-50 px-4 py-3 ring-1 ring-gray-950/5 dark:bg-white/5 dark:ring-white/10">
                    <span class="text-sm font-medium text-gray-950 dark:text-white">Всього до сплати</span>
